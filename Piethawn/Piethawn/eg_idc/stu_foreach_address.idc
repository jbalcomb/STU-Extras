...
static stu_foreach_address(void)
{
    auto ea_begin;
    auto ea_min;
    auto ea_max;
    ...
    // next_addr = NextAddr(ea_min);
    // if (next_addr == BADADDR)
        
    Message("\n\n------ Creating the output file.... --------\n");
    
    file_name = GetIdbPath();
    Message("file_name: %s\n", file_name);
    file_name = substr(file_name, 0, strstr(file_name, ".")) + "_addrs" + ".csv";
    Message("file_name: %s\n", file_name);
    file_handle = fopen(file_name, "w");
    file_cursor_position = ftell(file_handle);
    Message("file_cursor_position: %d\n", file_cursor_position);
    
    Message("addr_addr, addr_name_visible, addr_name_true, addr_flags, addr_type\n");
    
    writestr(file_handle, "addr_addr, addr_name_visible, addr_name_true, addr_flags, addr_type\n");
    
    // Segemnts seg001 through seg007; NOTE: 0x14787 is actually the start address of seg008
    //for(itr_ea_func_next = 0x13209; itr_ea_func_next < 0x14787; itr_ea_func_next = NextAddr(itr_ea_func_next))
    for(itr_ea_addr_next = ea_min; itr_ea_addr_next != BADADDR; itr_ea_addr_next = NextAddr(itr_ea_addr_next))
    {
        // Message("itr_ea_addr_next: %08X\n", itr_ea_addr_next);

        addr_name_visible = NameEx(0, itr_ea_addr_next);
        addr_name_true = GetTrueNameEx(0, itr_ea_addr_next);
        addr_type = GetType(itr_ea_addr_next);
        addr_flags = GetFlags(itr_ea_addr_next);
        
        Message("itr_ea_addr_next: 0x%08X, addr_name_visible: %s, addr_name_true: %s, addr_flags: 0x%016X, addr_type: %s\n", itr_ea_addr_next, addr_name_visible, addr_name_true, addr_flags, addr_type);
        
        fprintf(
            file_handle,
            "0x%08X, %s, %s, 0x%016X, %s\n",
            itr_ea_addr_next, addr_name_visible, addr_name_true, addr_flags, addr_type
        );
