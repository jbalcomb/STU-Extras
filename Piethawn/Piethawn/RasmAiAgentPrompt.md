...
Do not make up constant values. If a symbolic constant name is uncertain but the numeric value is clear, keep the numeric value and add a comment rather than inventing a cleaner-looking enum or #define with guessed numbering.
...
Do not invent functionality. If the assembly is ambiguous, show both possibilities in comments. If a code block's purpose is unclear, translate it mechanically and add /* TODO: purpose unclear */. This also applies to constants, enums, flags, table indices, and manifest values: preserve what the assembly proves and do not replace it with guessed abstractions.
Output Format
